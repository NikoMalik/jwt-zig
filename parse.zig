...
pub fn parseToken(allocator: Allocator, raw: []u8, signature: [64]u8) !jwt.Token {
    ...
    const header_struct = try head.unmarshalJSON_HEADER(allocator, header_buf.items);
    ...
    const payload_struct = try pl.unmarshalJSON_PAYLOAD(allocator, payload_buf.items);
